...
if (isset($_SESSION['db'])) 
	{
		...
		while ($rows = mysqli_fetch_array($sql)) 
			...
			$fk = 'FK';
				...
				$keynum = 1;
					$tbname = "";
				$rerspri = mysqli_query($db, "SELECT * FROM information_schema.KEY_COLUMN_USAGE
				WHERE TABLE_SCHEMA ='".$DatabaseName."'   AND TABLE_NAME ='".$rows[0]."';"); 
				$rersfk = mysqli_query($db, "SELECT * FROM information_schema.KEY_COLUMN_USAGE
				WHERE TABLE_SCHEMA ='".$DatabaseName."' AND CONSTRAINT_NAME <>'PRIMARY' AND CONSTRAINT_NAME LIKE  '%FK%'  AND TABLE_NAME ='".$rows[0]."';"); 
					while ($row_rs = mysqli_fetch_array($rs)) 
						{
							$rowddew = mysqli_fetch_assoc($rerspri);
						$rowddews = mysqli_fetch_assoc($rersfk);
								if($rowddew['CONSTRAINT_NAME'] == 'PRIMARY' && $row_rs['Field'] == $rowddew['COLUMN_NAME']) 
									{
										$pk = "PK(".$pknum.")";
										$pknum = $pknum +1;
									}
							...
							/*$fkname = $rowddews['CONSTRAINT_NAME'];
								
								if($row_rs['Field'] == $rowddews['COLUMN_NAME'] || $row_rs['Key'] =='MUL' && $fkname!=$rowddews['CONSTRAINT_NAME'])
									{
										 $fk = "FK(".$fknum.")";
										 $fknum = $fknum +1;
										 $fkname = $rowddews['CONSTRAINT_NAME'];
										 									}
								else if($row_rs['Field'] == $rowddews['COLUMN_NAME'] || $row_rs['Key'] =='MUL' && $fkname==$rowddews['CONSTRAINT_NAME'])
									{
									...
									$fkname = $rowddews['CONSTRAINT_NAME'];
										...
										if($row_rs['Field'] == $rowddews['COLUMN_NAME'])
									{
												$fkname = $rowddews['CONSTRAINT_NAME'];
												if($fkname==$rowddews['CONSTRAINT_NAME'])
												{
													$fk = "FK(".$fknum.")";
													$fknum = $fknum +1;
													$fkname = $rowddews['CONSTRAINT_NAME'];
												...
												else //Название колонки == название связи || Ключ = FK и предыдущий ключ не равен настоящему 
												{
													$fk = "FK".$fknum;
													$fknum = $fknum +1;
													$fkname = $rowddews['CONSTRAINT_NAME']; 
							...
							$listtable = mysqli_query($db, "SELECT    TABLE_NAME, REFERENCED_TABLE_NAME  FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE
							WHERE REFERENCED_TABLE_SCHEMA = '".$DatabaseName."' AND TABLE_NAME = '".$rows[0]."' AND REFERENCED_COLUMN_NAME = '".$row_rs['Field']."' ;");
								if($row_rs['Key'] == 'PRI' && $pknm ==1) 
									...
									while($rowlist = mysqli_fetch_array($listtable))
											{	
												...
												$fkmet==$rowlist['CONSTRAINT_NAME'];
												if($pos  == false && $fkmet==$rowlist['CONSTRAINT_NAME']) 
													{
														$key = "FK".$keynum;
														$fkmet=$rowlist['CONSTRAINT_NAME'];
														$keynum++;
											...
											$tbnames = "PK(".$rowlist['REFERENCED_TABLE_NAME'].")";
											
											$fkline==$rowlist['CONSTRAINT_NAME'];
												if($tbnames != $tbname )
													 {
															$tbname = "PK(".$rowlist['REFERENCED_TABLE_NAME'].")";
																	$tbnamess = $rowlist['REFERENCED_TABLE_NAME'];
												 ...
												 ELSE if($rowlist['TABLE_NAME'] == $rowlist['REFERENCED_TABLE_NAME'])
													 {
																								$tbname = "PK(".$rowlist['REFERENCED_TABLE_NAME'].")";
														$rowdt = array ( 'key' => $key , 'tbname' => $tbname);
														array_push($methodlist, $rowdt);
...
<div id="sample" class="main">
  <div id="myDiagramDiv" style="background:white; text-color:white; width:100%; height:1000px"></div>
